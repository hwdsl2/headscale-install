...
> [!IMPORTANT]
> `--serverurl` 選項僅用於設定 Headscale 向客戶端公布的伺服器 URL。它不會自動取得 TLS 憑證，也不會安裝或設定反向代理。你可以先執行安裝指令碼，但在客戶端透過該 HTTPS URL 連線前，必須先設定好 Caddy 或 nginx。

**提示：** 你可以選擇在同一台伺服器上安裝 [WireGuard](https://github.com/hwdsl2/wireguard-install/blob/master/README-zh-Hant.md)、[OpenVPN](https://github.com/hwdsl2/openvpn-install/blob/master/README-zh-Hant.md) 和/或 [IPsec VPN](https://github.com/hwdsl2/setup-ipsec-vpn/blob/master/README-zh-Hant.md)。
